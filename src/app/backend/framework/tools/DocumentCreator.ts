...
export class DocumentCreator {
   ...
   * @param TCreator 
   */
  static loadTo<T>(database: DatabaseCRUD_Interface, path_to_jsonfile:string, TCreator: new() => T, use_relation:boolean=false): void {
    ...
    fs.readJson(pathtofile)
      .then((packageObj: any): any => {
        for (const i in packageObj) {
          ...
          DocumentCreator.transportAndPersist(packageObj[i], obj, database, use_relation);
        }
        return null;
      })
      .catch((err: any) => {
  ...
  static transportAndPersist(
    ...
    database
      .create(obj, use_relation) // DocumentCreator.makeJsonObjectfromObject(obj)
