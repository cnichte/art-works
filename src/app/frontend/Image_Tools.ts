...
export default class ImageTools {
  ...
  resize = (file: any, maxDimensions: { width: any; height: any }) =>
    new Promise((resolve) => {
      ...
      image.onload = () => {
        ...
        EXIF.getData(image.baseURI, () => { // TODO img geht nicht mehr???
  ...
  crop = (file: any, dimensions: { width: any; height: any }) =>
    new Promise((resolve) => {
      if (!this.browserSupport || !file.type.match(/image.*/))
        return resolve(file); // early exit - not supported
      ...
      image.onload = () => {
        ...
        EXIF.getData(image.baseURI, () => { // TODO image geht nicht mehr
          const orientation = EXIF.getTag(image, 'Orientation');
          const imageCanvas = this.drawImageToCanvas(
            image,
            orientation,
  ...
  loadImage = (image: HTMLImageElement, file: Blob | MediaSource) => {
    if (typeof URL === 'undefined') {
      const reader = new FileReader();
      reader.onload = (event) => {
        // TODO image.src = event.target.result;
      };
