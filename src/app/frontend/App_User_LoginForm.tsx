      ...
      .then((result: DocUser[]) => {
        if (result.length == 0) {
          ...
          if (compareSync(formValues.password, result[0].password)) {
            // Der User hat sich erfolgreich angemeldet. Benachrichtige das Backend.
          ...
          App_Messages_IPC.request_message(
            "request:message-error",
            "Es gibt mehr als einen User mit diesem Username. Bitte den Administrator kontaktieren."
          );
        }
      })
      .catch(function (error: any) {
